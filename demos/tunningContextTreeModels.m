...
% 1, 186-209 (2012) this script execute the different tunning procedures
...
% indexes of the model choosen (for each sample and each of the 18 tunning
% configurations)
...
% % % %     [~, opt] = tunning_SMC(champions, A, n1, n2, alpha, b_blocks_g(:,1:n2), 0);
% % % %     idx_optimalModel(r,1) = opt;
% % % %     
% % % %     %BIC - df=fix - smc - block (computed renw. point)
% % % %     [~, opt] = tunning_SMC(champions, A, n1, n2, alpha, b_blocks_c(:,1:n2), 0);
% % % %     idx_optimalModel(r,2) = opt;
% % % %     
% % % %     %BIC - df=fix - smc - parametric
% % % %     [~, opt] = tunning_SMC(champions, A, n1, n2, alpha, b_param(:,1:n2), 0);
% % % %     idx_optimalModel(r,3) = opt;
% % % %     
% % % %     %BIC - df=fix - risk - block (given renw. point)
% % % %     opt = tunning_risk(prmvalues, b_blocks_g, A, options);
% % % %     idx_optimalModel(r,4) = opt;
% % % %     
% % % %     %BIC - df:fix - risk - block (computed renw. point)
% % % %     opt = tunning_risk(prmvalues, b_blocks_c, A, options);
% % % %     idx_optimalModel(r,5) = opt;
% % % %     
% % % %     %BIC - df:fix - risk - parametric
% % % %     opt = tunning_risk(prmvalues, b_param, A, options);
...
% % %     champions_bic_var{r} = champions; 
    ...
    % estimate the Champion Trees using Context Algorithm
    [champions, Ps, ~, prmvalues] = estimate_championTrees(X, A, 'MaxTreeHeight', max_height, ...
