function [past, M, Mc, iT] = contextTree_to_FiniteMarkov(contexts, P, A)
...
% looks for the posible transitions for each past
for p = 1 : n_past 
    if existing_past(p)
        l_p = I(p,:);
        ...
        M(p, ipastT) = P(iwp, :);
        Mc(p, :) = P(iwp, :);
        % matrix with indices of new_past = past + symbol
        iT(p, :) = ipastT;
    end
end
