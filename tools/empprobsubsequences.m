...
%EMPPROBSUBSEQUENCES Estimate the probability of occurence of all the
%                    sequences of length lower than height that can be
%                    generated by the context tree model. The transicion
...
%                  ocurrence of all posible sequences of lenght 1,2,...,height. 
...
for k = 2 : height - 1
    [past{k}, I] = permn(A, k);  % all possible past of lenght k
    for p = 1 : size(I,1)
...
% simulates a sequence acording to the context tree model 
...
% Find the index of the first ocurrence of subsequece of length <= k (update
...
for p = 2 : height
    idx_symbol = seq(p) + 1;
    est_P{1}(idx_symbol) = est_P{1}(idx_symbol) + 1;  % always update frec of symbols
    for j = 2 : p-1
